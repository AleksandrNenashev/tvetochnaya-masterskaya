...
class ModelExtensionPaymentYoomoney extends Model
{
    ...
    private $kassaModel;
    private $client;
    private $repository = 'cms/repos/cms-opencart3';
    ...
    public function createBackup($version)
    {
        ...
        $root            =  $reader->readFile($this->getFileName(), $sourceDirectory);
    ...
    public function downloadLastVersion($tag, $useCache = true)
    {
        ...
        if (!file_exists($dir)) {
            if (!mkdir($dir) && !is_dir($dir)) {
                $this->log('error', sprintf($this->language->get('updater_error_create_directory'), $dir));
        ...
        if (empty($fileName)) {
            $this->log('error', $this->language->get('updater_error_load'));
    ...
    {
        if (!file_exists($directoryName) && !mkdir($directoryName) && !is_dir($directoryName)) {
    ...
    {
        return dirname(__FILE__) . DIRECTORY_SEPARATOR . 'yoomoney' . DIRECTORY_SEPARATOR . $this->getMapFileName();
    }
}
