			...
			if ($category['top']) {
				...
				foreach ($children as $child) {
				    ...
				    foreach ($subchildren as $subchild) {
				        $subchildren_filter_data = array(
				            'filter_category_id'  => $subchild['category_id'],
				            'filter_sub_category' => true
				        );
				        
				        $subchildren_data[] = array(
				            'name'  => $subchild['name'] . ($this->config->get('config_product_count') ? ' (' . $this->model_catalog_product->getTotalProducts($subchildren_filter_data) . ')' : ''),
